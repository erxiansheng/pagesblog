...
- 🔀 文章手动序号排序（后台管理支持自定义排序序号，跨页排序无障碍）
- 🖼️ 封面图片点击放大预览
...
- 文章管理：创建、编辑、删除文章，Markdown 编辑器，手动序号排序（数字越小越靠前，支持跨页调整），分类筛选，分页浏览（每页 10 条）
- 分类管理：增删分类
- 评论管理：审核、删除评论
...
首页卡片区域只展示在后台编辑时勾选了「展示在首页」的文章，与排序位置无关。未勾选的文章不会出现在首页卡片中，但可以通过「查看全部文章」弹窗浏览。
